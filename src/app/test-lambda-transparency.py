...
def convert_black_cells_to_transparent(input_image):
    ...
    r,g,b,a = rgba.split()
    mask = Image.eval(r, lambda p:0 if p>10 else 255)
    ...
    for pixel_values in rgba_data:
        ...
        # Check for cells with black pixels (r=0,g=0,b=0)
        if [R,G,B] == [0,0,0]:
            # storing a transparent value when we find a black colour (Converting black to transparency layer)
            # print("{} = {}".format(coord, img_pixels))
            transparency_list.append((255, 255, 255, 0))
        else:
            # Not black - store as per normal
